...
KeePass2Reader::KeePass2Reader()
    ...
    , m_saveXml(false)
    , m_db(nullptr)
    , m_irsAlgo(static_cast<KeePass2::ProtectedStreamAlgo>(-1))
{
}
...
Database* KeePass2Reader::readDatabase(QIODevice* device, const CompositeKey& key, bool keepDatabase)
{
    ...
    if (m_version < KeePass2::FILE_VERSION_4) {
        SymmetricCipher::Algorithm cipher = KeePass2::cipherUuidToAlgo(m_db->cipher());
        if (cipher == -1) {
            raiseError("Unknown cipher");
            return nullptr;
        }
        SymmetricCipherStream* cipherStream = new SymmetricCipherStream(m_device, cipher,
                                                                        KeePass2::getAlgoMode(cipher), SymmetricCipher::Decrypt);
    ...
    KeePass2XmlReader xmlReader(m_version, binaryPool);
...
bool KeePass2Reader::readHeaderField(QIODevice* device)
{
    ...
    case KeePass2::KdfParameters: {
        ...
        m_db->setKdfParams(readVariantMap(&bufIoDevice));
        break;
    }
...
bool KeePass2Reader::readInnerHeaderField(QIODevice* device)
{
    ...
    default:
        qWarning("Unknown inner header field read: id=%d", fieldID);
        break;
    }
...
QVariantMap KeePass2Reader::readVariantMap(QIODevice* device)
{
    ...
    QVariantMap vm;
    while (true) {
        QByteArray fieldTypeArray = device->read(1);
        if (fieldTypeArray.size() != 1) {
            raiseError("Invalid variant map field type size");
            return QVariantMap();
        }
        KeePass2::VariantMapFieldType fieldType = static_cast<KeePass2::VariantMapFieldType>(fieldTypeArray.at(0));
        if (fieldType == KeePass2::VariantMapFieldType::End) {
            break;
        }
...
void KeePass2Reader::setCipher(const QByteArray& data)
{
    if (data.size() != Uuid::Length) {
        ...
        if (uuid != KeePass2::CIPHER_AES && uuid != KeePass2::CIPHER_CHACHA20) {
...
void KeePass2Reader::setAesTransformSeed(const QByteArray &data)
{
    if (data.size() != 32) {
        ...
        QByteArray uuidBytes = m_db->kdfParams().value(KeePass2::KDFPARAM_UUID).toByteArray();
        if (uuidBytes.size() != Uuid::Length || Uuid(uuidBytes) != KeePass2::KDF_AES) {
            m_db->setKdfParams(AesKdf().defaultParams());
        }

        m_db->kdfParams().insert(KeePass2::KDFPARAM_AES_SEED, QVariant(data));
    }
}
...
void KeePass2Reader::setAesTransformRounds(const QByteArray &data)
{
    if (data.size() != 8) {
        ...
        m_db->kdfParams().insert(KeePass2::KDFPARAM_AES_ROUNDS, QVariant(Endian::bytesToUInt64(data, KeePass2::BYTEORDER)));
    }
}
...
void KeePass2Reader::setInnerRandomStreamID(const QByteArray& data)
{
    if (data.size() != 4) {
        ...
        KeePass2::ProtectedStreamAlgo id = static_cast<KeePass2::ProtectedStreamAlgo>(Endian::bytesToUInt32(data, KeePass2::BYTEORDER));

        if (id == KeePass2::Salsa20 || id == KeePass2::ChaCha20) {
            m_irsAlgo = id;
        } else {
